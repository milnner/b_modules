...
func TestReadAreaIdsByOwnerId(t *testing.T) {
	var err error
	var dbConn *sql.DB
	...
	ctrl := readCtrl.NewReadAreaIdsByOwnerIdController(areaRepo, config.Logger, tkz)
	...
	if err := json.NewDecoder(wr.Body).Decode(&areasIds); err != nil {
		t.Fatal(err)
	}

	areasObjsIds := []int{1, 2, 3, 4}
	if !reflect.DeepEqual(areasIds, areasObjsIds) {
		t.Errorf("Not ok, areas: %v \nObjs:%v", areasIds, areasObjsIds)
	}
